...
The compound symmetric model was at one time a very popular choice for parametric covariance structure, specifying
...
\sqrt{i/j} &i \ne j \\ 
& \\
j\sigma_\epsilon^2 & i\ne j, \\
\end{array}\right.
\end{equation}
\noindent
This model is applicable to longitudinal data only when data are observed on a regular grid, however, its continuous time analogue permits this restriction to be relaxed. An important special case is the continuous time analogue to the random walk, the Weiner process, which has covariance function $Cov\left(\right) = \sigma^2 \min\left(t_i, t_j\right)$.

\bigskip

The formulation of the covariance structure in a random coefficient model is most usually a consideration of regressions that vary across subjects rather than a consideration of within-subject similarity, so this class of models are most often considered distinct from parametric covariance models. Still, they yield parametric covariance structures that generally have non-constant variances and non-stationary correlations.  A general form of the random coefficient model is given by 

\begin{equation}
y_i = X_i\beta + Z_i \gamma_i + \epsilon_i, \quad i = 1, \dots, M,
\end{equation}
\noindent
where the $Z_i$ are specified matrices, the $\gamma_i$ are vectors of random coefficients distributed independently as $N \left(0, G_i\right)$, the $G_i$ are positive definite but otherwise unstructured matrices, and the $\epsilon_i$ are distributed independently (of the $\gamma_i$ and of each other) as $N \left(0, \sigma^2 \mathrm{I}_{n_i}\right)$. The $G_i$ are usually assumed to be equal, so the covariance matrix of $y_i$ is taken to be $\Sigma_i = Z_i GZ'_i + \sigma^2 \mathrm{I}_{n_i}$. Special cases include the linear random coefficients (RCL) and quadratic random coefficients (RCQ) models. In the linear case, $Z_i = \left[1_{n_i} , t_i\right]$ and 
...
In the quadratic case, Zi = [1ni , ti, (t2i1, t2i2, . . . , t2ini )?]. Note also that the
CS structure is equivalent to the case Z i = 1ni .
Two convenient features of RC models are their parsimony (note that the number of parameters is unrelated to the number of measurement times) and their applicability to situations in which the measurement times are unequally spaced or non-rectangular.

This structure is flexible enough to permit several kinds of variance and correlational behavior, including increasing or decreasing variances, and correlations of which some are negative while others are positive. Observe, however, that it precludes the variance from being a concave-down function of time, and it precludes the variances from being constant if the same-lag correlations are not.
...
\end{equation}
\noindent
where $f_{0,p}\left(ï¿½\right) = 1$, $f_{j,p}\left(ï¿½\right)$, 1 ? j ? m, and ?p(ï¿½) are continuous functions on $\left[0, 1\right]$ and {?t}
