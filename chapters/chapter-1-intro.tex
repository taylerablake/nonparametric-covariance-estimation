...
from a p-variate N(µ, ?), the sample covariance matrix
S = (n ? 1)?1 Xn
i=1
(Yi ? Y¯ )(Yi ? Y¯ )
0
...
modeling the mean vector µ (first moments) of the distribution of a random vector Y =
...
g(·) and a linear predictor g(µ) = X? to induce unconstrained parameterization and reduce
the parameter space dimension simultaneously. Since the covariance matrix of a random
vector Y , defined by ? = E(Y ? µ)(Y ? µ)
0
