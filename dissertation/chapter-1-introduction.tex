...
{\needsparaphrased{TO DO: Refactor the next two paragraphs - need cohesive order and less stupid speech.
...
The development of the aforementioned regression framework allows those of us concerned with covariance estimation to stand on the shoulders of giants of sorts. However, the vast majority of the instruments in that toolbox require that the data exhibit certain structure - namely, repeated measurements be taken at regularly spaced observation times, and these observation times must be common across all subjects in the dataset. Instead of viewing the vectors of repeated measurements as realizations of a discrete random vector, we view each measurement as the observed value of a continuous random process at a specific point in time. Naturally, we extend this notion to generalized autoregressive parameters, interpreting them as the evaluation of a continuous bivariate function at the pairs of observed time points rather than specifying a finite set of observations to be multivariate normal and estimating the covariance matrix. This is advantageous because it is unlikely that we are only interested in the covariance between pairs of observed design points, so it is reasonable to approach covariance estimation in a way that allows us to obtain an estimate of the covariance between two measurements at any pair of time points within the time interval of interest. 
}}
\bigskip
