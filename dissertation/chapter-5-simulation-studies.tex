...
\tilde{\phi}\left(t,s\right) &= 0, \quad 0 \le s < t \le 1,\\[0.15cm] 
...
\tilde{\phi}\left(t,s\right) &= t - \frac{1}{2},  \quad 0 \le t \le 1, \\[0.15cm]
...
\multicolumn{2} {l} {III: Banded linear varying coefficient function, constant innovation variances} \\[0.3cm]
 $\Sigma = T^{-1} D {T'}^{-1}$ &$ \begin{aligned}
\tilde{\phi}\left(t,s\right) &= \left\{\begin{array}{ll} t - \frac{1}{2}, & t - s \le 0.5\\ 
...
\tilde{\phi}\left(t,s\right) &= \frac{\rho}{1 + \left(t-2\right)\rho}, \begin{array}{l} t = 2, \dots, p,\\ 
				 s = 1, \dots, t-1 \end{array}\\
\sigma^2\left(t\right) &= \left\{\begin{array}{ll} 1, & t = 1\\ 1 -\frac{\left(t-2\right)\rho^2}{1 + \left(t-2\right)\rho}, & t = 2, \dots, p \end{array}\right.
\end{aligned}$ \\
 \\
\hline
...
\]
\noindent 
where $s_{ij}$ denotes the $i$-$j^{th}$ entry of the sample covariance matrix, and $\lambda$ is a penalty parameter controlling the amount of shrinkage applied to the empirical estimator. 
...
y\left(t_{ij}\right) = \sum_{k < j} \left(\beta_0 + \beta_1 t_{ik}\right) y\left( t_{ik} \right) + \epsilon\left(t_{ij}\right),
\end{equation}
\noindent 
where $\epsilon\left(t_{ij}\right)$ are independent mean zero Normal random variables with common variance $\sigma^2$. The estimator of $\bfbeta = \left(\beta_0, \beta_1\right)'$, $\hat{\bfbeta}$ is taken to be 
...
\]
\noindent
The estimator for $\sigma^2$ is then the mean of the squared residuals:
\[
\hat{\sigma}^2 = \frac{1}{N}\left[\sum_{i = 1}^N\frac{1}{\left(p_i - 1\right)} \sum_{j = 1}^{p_i} e_{ij}^2\right],
\]
\noindent
...
y\left(t_{ij}\right) = \sum_{ t_{ij} - t_{ik} < 0.5} \left(\beta_0 + \beta_1 t_{ik}\right) y\left( t_{ik} \right) + \epsilon\left(t_{ij}\right). 
\end{equation}
...
\end{align*}
\noindent
where $Y_1,\dots, Y_N$ are independently and identically distributed according to an $p$-dimensional multivariate Normal distribution with mean zero and having covariance structure identical to one of Models I - V in Table~\ref{table:simulation-model-list}. To induce sparsity, we subsample from the complete data $\left\{y_i\left(t_j\right) \right\}$, $i = 1,\dots, N$, $j = 1,\dots, p$, randomly omitting an observation $y_i\left(t_j\right)$ with probability $0.1$, $0.2$, and $0.3$. For both sets of simulations, the smoothing parameters for the smoothing spline and P-spline estimators were selected using both leave-one-subject-out cross validation $\mbox{losoCV}\left(\lambda\right)$ and unbiased risk estimate $\mbox{U}\left(\lambda\right)$. Given the selected values of the tuning parameters, we computed the estimated covariance matrix and compared it to the true covariance matrix via entropy loss and quadratic loss. 
...
\begin{figure}[H] 
  ...
  \caption{\textit{Estimated functional components of the smoothing spline ANOVA decomposition $\phi = \phi_1 + \phi_2 + \phi_{12}$ for $\hat{\Sigma}_{SS}$ under each simulation Model I - V.}}\label{fig:ssanova-component-lattice}
\end{figure}
...
\captionsetup[subfigure]{labelformat=empty}
  \centering
  \subfloat[]{\includegraphics[width=0.75\textwidth]{img/chapter-4/model-1-entropy-loss-boxplots}}\hfill
    \subfloat[]{\includegraphics[width=0.75\textwidth]{img/chapter-4/model-1-entropy-loss-boxplots-zoom}}
      \vspace*{3mm}
  \caption{\textit{The distribution of the square root of entropy loss $\Delta_2$ under Model I for each of the estimators considered in the simulations with complete data. The top figure displays results under each of the three data dimensions $p = 10, 20, 30$ and sample sizes $N = 50, 100$. The relative performance of each estimator compared to the others is consistent across dimensions. The bottom figure shows a more detailed view for comparison between the oracle estimator and the estimators based on the modified Cholesky decomposition.}}  \label{fig:model-1-entropy-loss-boxplots}
\end{figure}

\begin{figure}[H] 
\centering
\includegraphics[width =\textwidth]{img/chapter-4/model-2-entropy-loss-boxplots-p-20}
  \vspace*{3mm}
  \caption{\textit{The distribution of the square root of entropy loss $\Delta_2$ under Model III for the smoothing spline estimator $\hat{\Sigma}_{SS}$ for 100 simulated multivariate normal samples of size $N = 50$ (hollow boxplots) and $N = 100$ (filled boxplots) with dimension $p = 20$. The estimator $\Sigma_{poly}$ based on the parametric model for the modified Cholesky decomposition suffers from model misspecification, which is clearly reflected in its performance as measured by $\Delta_2$.}}  \label{fig:model-2-entropy-loss-boxplots}
\end{figure}

\begin{figure}[H] 
\centering
\includegraphics[width =\textwidth]{img/chapter-4/model-3-entropy-loss-boxplots-zoom}
  \vspace*{3mm}
  \caption{\textit{The distribution of the square root of entropy loss $\Delta_2$ under Model III for the smoothing spline estimator $\hat{\Sigma}_{SS}$ for 100 simulated multivariate normal samples of size $N = 50$ (hollow boxplots) and $N = 100$ (filled boxplots) with dimension $p = 20$. The loss distributions are shown for the sample covariance matrix as well as the tapered sample covariance matrix. While the underlying inverse covariance matrix is banded, the covariance matrix itself is not, which explains why $S$ outperforms its regularized variant $S_\omega$.}}  \label{fig:model-3-entropy-loss-boxplots}
\end{figure}

\begin{figure}[H] 
\centering
\includegraphics[width =\textwidth]{img/chapter-4/model-4-entropy-loss-boxplots-zoom}
  \vspace*{3mm}
  \caption{\textit{The distribution of the square root of entropy loss $\Delta_2$ under Model IV for the smoothing spline estimator $\hat{\Sigma}_{SS}$ for 100 simulated multivariate normal samples of size $N = 50$ (hollow boxplots) and $N = 100$ (filled boxplots) with dimension $p = 20$. The performance of the estimator $\Sigma_{poly}$ based on the parametric model for the modified Cholesky decomposition is more comparable to that of our estimators when the underlying covariance matrix is stationary.}}  \label{fig:model-4-entropy-loss-boxplots}
\end{figure}

\begin{figure}[H] 
\centering
\includegraphics[width =\textwidth]{img/chapter-4/model-5-entropy-loss-boxplots-zoom}
  \vspace*{3mm}
  \caption{\textit{The distribution of the square root of entropy loss $\Delta_2$ under Model V for the smoothing spline estimator $\hat{\Sigma}_{SS}$ for 100 simulated multivariate normal samples of size $N = 50$ (hollow boxplots) and $N = 100$ (filled boxplots) with dimension $p = 20$. The parsimony of the compound symmetric model, having only two parameters to be estimated, lends to the marked improvement of the performance of the sample covariance when the sample size is increased from $N = 50$ to $100$.}}  \label{fig:model-5-entropy-loss-boxplots}
\end{figure}
...
\centering
  \begin{tikzpicture}
    \draw (0,0) node {  \includegraphics[width = \textwidth]{img/chapter-4/simulation-study-2-entropy-loss-boxplots} };
% \node at (-2,3) {\textcolor{wes1}{$p = 20$}};
\draw[red] (-6.3,-.7) node {{$p = 20$}}; % Draws a circle
\draw[red] (-6.3,4.2) node {{${p = 10}$}}; % Draws a circle
\end{tikzpicture} \caption{\textit{The distribution of entropy loss for the smoothing spline estimator $\hat{\Sigma}_{SS}$ for 100 simulated multivariate
                            normal samples of size $N = 50$ when 0\%, 10\%, 20\%, and 30\% of the data are missing for each subject for dimension $p = 10$ (top) and $p = 20$ (bottom). }}\label{fig:simulation-study-2-boxplots}
\end{figure}
...
\begin{table}[H]
\centering
\caption{\textit{Model I: Entropy risk estimates and corresponding standard errors 
...
\begin{table}[H]
\centering
\caption{\textit{Model II: Entropy risk estimates and corresponding standard errors.} }
...
\begin{table}[H]
\centering
\caption{\textit{Model III: Entropy risk estimates and corresponding standard errors.} }
...
\begin{table}[H]
\centering
\caption{\textit{Model IV: Entropy risk estimates and corresponding standard errors.} }
...
\begin{table}[H]
\centering
\caption{\textit{Model V: Entropy risk estimates and corresponding standard errors.} }
