...
{\needsparaphrased{TO DO: clean this up, omitting all of the non-varying coefficient model discussion; move the pertinant B-spline discussion to the appendix. }}
...
\]
\noindent
and the elements of $B$ are given by $b_{ij} = B_j\left(t_i\right)$, as defined in \ref{eq:S_varying_intercept_model}. Taking derivatives on both sides of \ref{eq:S_pen_varying_intercept_model} with respect to $\alpha$ and setting equal to zero yields normal equations: 
...
which has explicit solution
\[
\hat{\alpha} = \left(B' B +  \lambda D_d'D_d\right)^{-1}B' y
\]
\noindent
The effective hat matrix is now
\[
H_\lambda - B\left(B^T B +  \lambda D_k^TD_k\right)^{-1}B'. 
\]
